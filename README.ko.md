...
❌/➖로 표시된 두 줄은 의도된 설계입니다. 둘 다 상시 게이트웨이 연결이나 메시지 내용 접근 권한이 필요한데, ghcord의 설계는 이를 금지합니다 — 게이트웨이 연결에 의존했던 선배 봇들은 결국 하나같이 유지보수 부담으로 죽었습니다.
...
피처 토큰은 GitHub for Slack의 어휘를 그대로 따릅니다:
...
빈칸을 채우는 일 — GitHub App, Discord 앱, HTTPS 엔드포인트 — 은 [설치 가이드](docs/ko/README.md)가 여섯 단계로 안내합니다:
